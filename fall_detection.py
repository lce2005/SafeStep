...
from predict import is_stair  # 계단 분류 팀원이 준 함수
...
def main():
    ...
    fall_counter = 0
    fall_flag = False
    alert_sent = False  # TTS/STT 호출 중복 방지 플래그
    ...
    while True:
        ...
        if stair_detected:
            ...
            if res.pose_landmarks:
                ...
                vel = 0
                if len(y_hist) >= 2:
                    vel = (y_hist[-1] - y_hist[-2]) * fps  # 초당 y 변화량
                ...
                cv2.rectangle(frame, (int(x_min * w), int(y_min * h)),
                              (int(x_max * w), int(y_max * h)), (255, 0, 0), 2)
                cv2.putText(frame, f"vel: {vel:.3f}", (20, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(frame, f"tilt: {tilt:.1f}", (20, 60),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(frame, f"aspect: {aspect:.2f}", (20, 90),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        else:
            # 평지 → 계단 진입 시 속도 오차 방지
            y_hist.clear()
        ...
        # 4) 응급 모듈 연동용 훅 (fall_flag True일 때 1회만)
        if fall_flag and not alert_sent:
            # >>> 여기서 TTS/STT 호출, 119/보호자 알림 연동 <<<
            alert_sent = True
        elif not fall_flag:
            alert_sent = False
        ...
        cv2.putText(frame, f"counter: {fall_counter}/{FALL_FRAMES}", (20, 170),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

        cv2.imshow("SafeStep Integrated System", frame)

        key = cv2.waitKey(1) & 0xFF
        if key == ord('q') or key == 27:  # q 또는 ESC
            break
        if cv2.getWindowProperty("SafeStep Integrated System", cv2.WND_PROP_VISIBLE) < 1:
            break
