...
```mermaid
flowchart TD
    Start["낙상 이벤트 발생"] --> T1["TTS: 괜찮으세요?"]
    T1 --> S1["STT: 사용자 1차 응답 인식"]
    
    S1 --> C1{"사용자 상태 판단"}
    
    C1 -->|괜찮음| E1["병원 방문 권유 후 종료"]
    C1 -->|괜찮지 않음| T2["TTS: 119에 신고해 드릴까요?"]
    C1 -->|무응답| D1{"위험/비위험 장소 판별"}
    
    T2 --> S2["STT: 119 신고 의사 확인"]
    
    S2 --> C2{"신고 의사 판단"}
    
    C2 -->|신고 요청| R1["119 신고 접수"]
    C2 -->|신고 거절| E2["절차 종료"]
    C2 -->|무응답| D1
    
    D1 -->|위험 장소| R1
    D1 -->|비위험 장소| M1["3분간 움직임 모니터링"]
